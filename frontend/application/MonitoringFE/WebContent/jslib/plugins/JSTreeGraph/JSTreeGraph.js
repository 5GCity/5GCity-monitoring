$.JSTreeGraph = {
		...
		center : function(container) {
			...
			var innerWidth = this.maxLeft + 160;
			var innerHeight = this.maxTop + 40;
			if (innerWidth < containerWidth) {
				this.centerOffsetLeft = (containerWidth - innerWidth) / 2;
			} else {
				...
				wrapper.mCustomScrollbar('scrollTo',(innerWidth - wrapper.width())/2);
			}
			if (innerHeight < containerHeight) {
				this.centerOffsetTop = (containerHeight - innerHeight) / 2;
			}
...
function DrawTree(options) {
	...
	// Center Node (Horizontal only)
	$.JSTreeGraph.center(options.Container);
	
	MoveRigth([ options.RootNode ], $.JSTreeGraph.centerOffsetLeft);
	MoveBottom([ options.RootNode ], $.JSTreeGraph.centerOffsetTop);
...
function DrawLines(node, container) {
	if ((!node.Collapsed) && node.Nodes && node.Nodes.length > 0) { // Has
		// children
		// and Is
		// Expanded
...
function DrawLineH(container, startPoint, endPoint) {
	var midY = (startPoint.Y + ((endPoint.Y - startPoint.Y) / 2)); // Half path
	// between
	// start en
	// end Y
	// point
	...
	// The higher value will be the ending point
...
function DrawLineV(container, startPoint, endPoint) {
	var midX = (startPoint.X + ((endPoint.X - startPoint.X) / 2)); // Half path
	// between
	// start en
	// end X
	// point
	...
	// Intermidiate segment
	var imsStartY = startPoint.Y < endPoint.Y ? startPoint.Y : endPoint.Y; // The
	// lower
	// value
	// will
	// be
	...
	// starting
	// point
	var imsEndY = startPoint.Y > endPoint.Y ? startPoint.Y : endPoint.Y; // The
	// higher
	// value
	// will
	// be
...
function DrawNode(node, container, options) {
	...
	if (node.Content)
		nodeDiv.innerHTML = node.Content;
	...
	/*
	 * nodeDiv.onmouseover = function () { // In this.PrevClassName =
	 * this.className; this.className = "NodeHover"; };
	 * 
	 * nodeDiv.onmouseout = function () { // Out if (this.PrevClassName) {
	 * this.className = this.PrevClassName; this.PrevClassName = null; } };
	 */
...
function PerformLayoutV(node) {
	...
	if ((!node.Collapsed) && node.Nodes && node.Nodes.length > 0) { // If Has
		// Children
		// and Is
		// Expanded

		// My Top is in the center of my children
		var childrenHeight = (node.Nodes[node.Nodes.length - 1].Top + node.Nodes[node.Nodes.length - 1].Height)
				- node.Nodes[0].Top;
	...
	// Child: Where the lines get out from to connect this node with its
	// children
...
function PerformLayoutH(node) {
	...
	if ((!node.Collapsed) && node.Nodes && node.Nodes.length > 0) { // If Has
		...
		var childrenWidth = (node.Nodes[node.Nodes.length - 1].Left + node.Nodes[node.Nodes.length - 1].Width)
				- node.Nodes[0].Left;
	...
	node.Top = (nodeMarginTop * node.Level) + (nodeHeight * node.Level);
	
	if (node.Left > $.JSTreeGraph.maxLeft) {
		$.JSTreeGraph.maxLeft = node.Left;
	}
	if (node.Top > $.JSTreeGraph.maxTop) {
		$.JSTreeGraph.maxTop = node.Top;
	}
...
function PrepareNode(node, level, parentNode, leftNode, rightLimits) {
	...
	if (level > $.JSTreeGraph.maxLevel) {
		$.JSTreeGraph.maxLevel = level;
	}
