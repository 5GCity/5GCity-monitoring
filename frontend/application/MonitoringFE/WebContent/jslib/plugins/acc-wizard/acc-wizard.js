    ...
    function Plugin(element, options) {
        ...
        function makeTaskActive(hash) {
            if (hash && hash.length > 1) {
            	...
            	var ctrl = hash.substring(1);
            	
            	$(options.sidebar,$el).find(".acc-wizard-task")
                .children("a[aria-controls='" + ctrl + "']").attr('href',hash);
            	
                $(options.sidebar,$el).find(".acc-wizard-task")
                    .children("a[href='" + hash + "']")
                    .parent(".acc-wizard-task").addClass(options.activeClass);

                // Remove class from other tasks
                $(options.sidebar,$el).find(".acc-wizard-task")
                    .children("a[href!='" + hash + "']")
                    .parent(".acc-wizard-task").removeClass(options.activeClass);
            }
        }
        ...
        function init() {
            ...
            if (options.addButtons) {
                ...
                // and count them.
                var forms = $(".panel-collapse ."+options.panelBodyClass+" form", $el);
            ...
            currentHash = currentHash ||
                          $(options.sidebar,$el)
                              .find(".acc-wizard-task."+options.todoClass).first()
                              .children("a").attr("href") ||
                          $(options.sidebar,$el)
                              .find(".acc-wizard-task").first()
                              .children("a").attr("href");
            ...
            var parent = "#" + $("."+options.panelGroupClass,$el)[0].id;
...
//            });
            ...
            // the vertical navigation task list to make
            // the current panel the active task.
            $("."+options.panelTitleClass+" a").on("click", function () {
    ...
    $.fn[pluginName].defaults = {
        ...
        panelBodyClass:	"box-body",				// AdminLTE 
        panelGroupClass:"box-group",			// AdminLTE
        panelTitleClass:"box-title",			// AdminLTE
        autoScrolling:  true,
