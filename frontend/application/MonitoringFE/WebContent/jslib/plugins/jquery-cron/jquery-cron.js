...
(function($) {
	...
	// CUSTOM : DOW = 1-7 (was 0-6)
	/*
	 * Riporta malf MALF ATCA100208441, risolta sul cloud
	 * 
	 ┌────────── minute (0 - 59)
	 │ ┌──────── hour (0 - 23)
	 │ │ ┌────── day of month (1 - 31)
		...
		if (defined(o.customValues)) {
			for (key in o.customValues) {
				if (combinations.hasOwnProperty(key)) {
					$.error("cron: reserved keyword '" + key + "' should not be used as customValues key.");
					return true;
				}
			}
		}
	...
	var methods = {
		init : function(opts) {
			...
			block["period"] = $(
					"<span class='cron-period'>" + "<span class='cron-addon'>Every</span><select name='cron-period'>" + custom_periods
							+ str_opt_period + "</select></span>").appendTo(this).data("root", this);
			...
			block["dom"] = $(
					"<span class='cron-block cron-block-dom'>" + "<span class='cron-addon'>on the</span><select name='cron-dom'>" + str_opt_dom
			...
			block["month"] = $(
					"<span class='cron-block cron-block-month'>" + "<span class='cron-addon'>of</span><select name='cron-month'>" + str_opt_month
							+ "</select></span>").appendTo(this).data("root", this);
			...
			block["mins"] = $(
					"<span class='cron-block cron-block-mins'>" + "<span class='cron-addon'>at</span><select name='cron-mins'>" + str_opt_mih
							+ "</select><span class='cron-addon'>minutes past the hour</span></span>").appendTo(this).data("root", this);
			...
			block["dow"] = $(
					"<span class='cron-block cron-block-dow'>" + "<span class='cron-addon'>on</span><select name='cron-dow'>" + str_opt_dow
							+ "</select></span>").appendTo(this).data("root", this);
			...
			block["time"] = $(
					"<span class='cron-block cron-block-time'>"
							+ "<span class='cron-addon'>at</span><select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
							+ "</select><span class='cron-addon cron-time-addon'>:</span><select name='cron-time-min' class='cron-time-min'>" + str_opt_mih
			...
			block["controls"] = $(
					"<span class='cron-controls'>&laquo; save " + "<span class='cron-button cron-button-save'></span>"
							+ " </span>").appendTo(this).data("root", this).find("span.cron-button-save").bind(
					"click.cron", event_handlers.saveClicked).data("root", this).end();

			this.find("select").bind("change.cron-callback", event_handlers.somethingChanged);
			this.data("options", o).data("block", block); // store options and
															// block pointer
			this.data("current_value", o.initial); // remember base value to
													// detect changes
	...
	var event_handlers = {
		...
		somethingChanged : function() {
			...
			if (defined(root.data("options").url_set)) {
				if (methods.value.call(root) != root.data("current_value")) { // if
																				// changed
