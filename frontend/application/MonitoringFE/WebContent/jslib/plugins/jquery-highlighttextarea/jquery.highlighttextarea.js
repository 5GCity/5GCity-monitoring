...
!function($) {
	...
	var Highlighter = function($el, options) {
		...
		this.$main
				.prepend('<div class="highlightTextarea-container"><div class="highlightTextarea-highlighter"></div></div>');
	...
	Highlighter.prototype.highlight = function() {
		// var text = Utilities.htmlEntities(this.$el.val()), that = this;
		var text = this.$el.val(), that = this;
		that.spacer = '';
		...
		$.each(this.settings.words, function(color, words) {
			text = text.replace(new RegExp(that.spacer + '(' + words.join('|') + ')' + that.spacer, that.regParam),
					function(match) {
						return '<mark style="background-color:' + color + ';">' + Utilities.htmlEntities(match) + '</mark>';
					});
		});
		...
		var start = 0;
		while(true) {
			var indexOfMarkStart = text.indexOf('<mark ', start);
			if (indexOfMarkStart < 0) {
				var postSubstring = text.substring(start, text.length);
				escapedText += Utilities.htmlEntities(postSubstring);
				break;
			}
			var preSubstring = text.substring(start, indexOfMarkStart);
			escapedText += Utilities.htmlEntities(preSubstring);
			var indexOfMarkEnd = text.indexOf('</mark>', indexOfMarkStart);
			var end = indexOfMarkEnd + '</mark>'.length;
			var markSubstring = text.substring(indexOfMarkStart, end);
			escapedText += markSubstring;
			start = end;
		}
		...
		this.$highlighter.html(escapedText);
		this.updateSizePosition();
	};

	/*
	 * Change highlighted words @param words {mixed}
	 */
	...
	/*
	 * Change highlighted ranges @param ranges {mixed}
	 */
	...
	// ===============================
	/*
	 * Change options @param options {object}
	 */
	...
	Highlighter.prototype.bindEvents = function() {
		...
		if (this.isInput) {
			this.$el.on({
				// Prevent Cmd-Left Arrow and Cmd-Right Arrow on Mac strange
				// behavior
				...
				// Force Chrome behavior on all browsers: reset input position
				// on blur
	...
	Highlighter.prototype.updateCss = function() {
		// the main container has the same size and position than the original
		// textarea
		Utilities.cloneCss(this.$el, this.$main, [ 'float', 'vertical-align' ]);
		this.$main.css({
			'width' : this.$el.outerWidth(true),
			'height' : this.$el.outerHeight(true)
		});

		// the highlighter container is positionned at "real" top-left corner of
		// the textarea and takes its background
		Utilities.cloneCss(this.$el, this.$container, [ 'background-image', 'background-color', 'background-position',
				'background-repeat', 'background-origin', 'background-clip', 'background-size',
				'background-attachment', 'padding-top', 'padding-right', 'padding-bottom', 'padding-left' ]);
		this.$container.css({
		...
		// the highlighter has the same size than the "inner" textarea and must
		// have the same font properties
		Utilities.cloneCss(this.$el, this.$highlighter, [ 'font-size', 'font-family', 'font-style', 'font-weight',
				'font-variant', 'font-stretch', 'line-height', 'vertical-align', 'word-spacing', 'text-align',
				'letter-spacing', 'text-rendering' ]);
	...
	/*
	 * Update size and position of the highlighter @param forced {boolean} true
	 * to resize containers
	 */
	Highlighter.prototype.updateSizePosition = function(forced) {
		...
		if (!this.isInput) {
			...
			if (this.$el.css('overflow') == 'scroll'
					...
					|| (this.$el.css('overflow') != 'hidden' && this.$el.css('overflow-y') != 'hidden' && this.$el[0].clientHeight < this.$el[0].scrollHeight)) {
				padding = this.scrollbarWidth;
			}

			width = this.$el.width() - padding;
		} else {
			// TODO: There's got to be a better way of going about this than
			// just using 99999px...
			width = 99999;
		}
	...
	Utilities.getScrollbarWidth = function() {
		var parent = $('<div style="width:50px;height:50px;overflow:auto"><div>&nbsp;</div></div>').appendTo('body'), child = parent
				.children(), width = child.innerWidth() - child.height(100).innerWidth();
	...
	/*
	 * Copy a list of CSS properties from one object to another @param from
	 * {jQuery} @param to {jQuery} @param what {string[]}
	 */
	...
	/*
	 * Convert a size value to pixels value @param value {mixed} @return {int}
	 */
	Utilities.toPx = function(value) {
		if (value != value.replace('em', '')) {
			var el = $('<div style="font-size:1em;margin:0;padding:0;height:auto;line-height:1;border:0;">&nbsp;</div>')
					.appendTo('body');
	...
	/*
	 * Converts HTMl entities @param str {string} @return {string}
	 */
	...
	/*
	 * Inserts a string in another string at given position @param string
	 * {string} @param index {int} @param value {string} @return {string}
	 */
	...
	/*
	 * Apply throttling to a callback @param callback {function} @param delay
	 * {int} milliseconds @param context {object|null} @return {function}
	 */
	 ...
	 * @param words {mixed} @param color {string} default color @return
	 * {object[]}
	 */
		...
		for (var i = 0, l = words.length; i < l; i++) {
			...
			if ($.isPlainObject(group)) {
				...
				for (var j = 0, m = group.words.length; j < m; j++) {
					...
					out[group.color].push(group.words[j]);
				}
			} else {
				if (!out[color]) {
					out[color] = [];
	 ...
	 * @param ranges {mixed} @param color {string} default color @return
	 * {object[]}
	 */
	...
	$.fn.highlightTextarea = function(option) {
		...
		return this.each(function() {
			var $this = $(this), data = $this.data('highlighter'), options = typeof option == 'object' && option;
