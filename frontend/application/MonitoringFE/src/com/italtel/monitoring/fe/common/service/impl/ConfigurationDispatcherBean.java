...
public class ConfigurationDispatcherBean implements
	...
	public void init() {
		...
		if (em.createQuery(QUERY_CHECK_SLICE).getResultList().isEmpty()) {
			...
			} else log.error("%%% initialize slice table not done%%%");
		}
		...
		if (em.createQuery(QUERY_CHECK_JOB).getResultList().isEmpty()) {
			...
			} else log.error("%%% initialize job table not done%%%");
		}
		// service
	...
	public <T extends OMEntity> T getDefault(Class<T> clazz) throws Exception {
		...
		if (Slice.class.getName().toString()
				.equals(clazz.getName().toString())) {
			return (T) new Slice();
		}		
		...
		if (InventoryService.class.isInstance(obj)) {
			...
			if (service != null) {
				...
				if (serviceName.equals(InventoryService.DEF_SERVICE_MONITORING)) {
					throw new ConfigException(
							"Service name ServiceMonitoring cannot be created");
				}
		...
		if (Slice.class.isInstance(obj)) {
			...
			if (slice != null) {
				...
				if (sliceName == null || sliceName.isEmpty()) {
					throw new ConfigException(
							"Slice name is null or empty");
				}
				if (sliceName.equals(Slice.DEF_SLICE_MONITORING)) {
					throw new ConfigException(
							"Slice name Monitoring cannot be created");
				}
		...
		if (Job.class.isInstance(obj)) {
			...
			if (job != null) {
				...
				if (jobName == null || jobName.isEmpty()) {
					throw new ConfigException(
							"Job name is null or empty");
				}
				if (jobName.equals(Job.DEF_JOB_MONITORING)) {
					throw new ConfigException(
							"Job name Monitoring cannot be created");
				}
...
//							"DashboardType cannot be null");
				...
				if(jobSource.getPort() != null) {
					if ((jobSource.getPort() < 0) || (jobSource.getPort() > Source.DEF_SOURCE_MAX_PORT)) {
						throw new ConfigException(
								"Job port is out of range (" + 0 + "-"
										+ Source.DEF_SOURCE_MAX_PORT + ")");
					}
				} else {
				...
				if(jobSource.getInterval() != null) {
					if ((jobSource.getInterval() < 0) || (jobSource.getInterval() > Source.DEF_SOURCE_MAX_INTERVAL)) {
						throw new ConfigException(
								"Job interval is out of range (" + 0
								+ "-" + Source.DEF_SOURCE_MAX_INTERVAL + ")");
					}
				} else {
					throw new ConfigException(
	...
	public void manageNodesInSlice(Slice slice)
	...
	public void manageJobsInService(InventoryService service)
			throws ConfigException {
		List<InventoryServiceJob> serviceJobs = service
				.getJobs();
	...
	public void manageNodesInJob(Job job)
			throws ConfigException {
		List<JobInventoryNode> jobNodes = job
				.getJobNodes();
	...
	public void validateNodesInJob(
			List<JobInventoryNode> jobNodes,
	...
	public void validateNodesInSlice(
			List<SliceInventoryNode> sliceNodes,
			Set<String> inventoryNodeNames) throws ConfigException {
	...
	public void validateJobsInService(
			List<InventoryServiceJob> serviceJobs,
			Set<String> jobNames,String nameService) throws ConfigException {
		for (InventoryServiceJob sn : serviceJobs) {
			...
			if(jobName.equals(Job.DEF_JOB_MONITORING)) {
				throw new ConfigException(
						"Job with name '" + jobName + "'" + "cannot be associated to service different to ServiceMonitoring");
			}
			
			Map<String, Object> queryParams2 = new HashMap<String, Object>();
			queryParams2.put("name", jobName);

			Job result = DBUtility
					.getNamedQuerySingleResultByNamedParams(em,
							Job.QUERY_READ_JOB, Job.class,
							queryParams2);
			if (result == null) {
			...
			if (!jobNames.add(jobName)) {
				throw new ConfigException(
						"duplicated job with name '" + jobName + "'");
			}
			
			// cerco in tutti i servivi se il mio nodo � gia' associato
			List<InventoryService> sl = DBUtility.getNamedQueryResultList(
					em,
				...
				if(!serv.getName().equals(nameService)) {
					...
					for (InventoryServiceJob job : jobsList) {
						if(job.getJobName().equals(jobName)) {
							...
							throw new ConfigException(
									"Job already associated to another service");
						}
					}
				}
	...
	protected <T extends OMEntity> void regeneratePrometheusConfig(boolean restart)
		...
		//		se sourceType ==  EXPORTER resta uguale, da aggiungere oltre all'intervallo anche
		//				metrics_path: /metrics (esempio)
		//				la porta diventa quella del source 
		//				
		//		se sourceType ==  GATEWAY � uguale ad exporter, l'indirizzo ip siamo noi stessi, la porta le
	...
	public <T extends OMEntity> T delete(T obj) throws Exception {
		...
		if (InventoryNode.class.isInstance(obj)) {
			...
			Long counter = DBUtility
					.getNamedQuerySingleResultByName(
							em,
							SliceInventoryNode.QUERY_COUNT_SLICENODE_WITH_NODE,
							Long.class, nodeName);
			if (counter > 0) {
				throw new ConfigException(
						"Node referenced by a slice cannot be deleted!");
			}
			Long counterJob = DBUtility
					.getNamedQuerySingleResultByName(
							em,
							JobInventoryNode.QUERY_COUNT_JOBNODE_WITH_NODE,
							Long.class, nodeName);
			if (counterJob > 0) {
				throw new ConfigException(
						"Node referenced by a job cannot be deleted!");
			}
		}
		if (Slice.class.isInstance(obj)) {
			Slice slice = (Slice) obj;
			...
			if (sliceName.equals(Slice.DEF_SLICE_MONITORING)) {
			throw new ConfigException(
					"Slice Monitoring cannot be deleted");
			}
			Long counter = DBUtility
					.getNamedQuerySingleResultByName(
							em,
							UserSlice.QUERY_COUNT_USERSLICE_WITH_SLICE,
							Long.class, sliceName);
			if (counter > 0) {
				throw new ConfigException(
						"Slice referenced by a user cannot be deleted!");
			}
		}
		...
		if (Job.class.isInstance(obj)) {
			...
			if (jobName.equals(Job.DEF_JOB_MONITORING)) {
				throw new ConfigException(
						"Job Monitoring cannot be deleted");
			}
			Long counter = DBUtility
					.getNamedQuerySingleResultByName(
							em,
							InventoryServiceJob.QUERY_COUNT_SERVICEJOB_WITH_JOB,
							Long.class, jobName);
			if (counter > 0) {
				throw new ConfigException(
						"Job referenced by a service cannot be deleted!");
			}
		}
		...
		if (Slice.class.isInstance(obj)) {
			...
			if (slice != null) {
				if (slice.getName() != null) {
					...
					if (slice.getName().equals(Slice.DEF_SLICE_MONITORING)) {
						log.error("Slice identified by name {} cannot be updated!",
								slice.getName());
					...
					Slice oldSl = DBUtility
							.getNamedQuerySingleResultByNamedParams(em,
									Slice.QUERY_READ_SLICE,
									Slice.class, queryParams1);

					if (oldSl == null) {
						log.error("Slice identified by name {} not found!",
								slice.getName());
			...
			if (service != null) {
				if (service.getName() != null) {
					...
					if (!(srvJobNames.containsAll(oldJobNames) && oldJobNames
							.containsAll(srvJobNames))) {
			...
			if (job != null) {
				if (job.getName() != null) {
					...
					if (job.getName().equals(Job.DEF_JOB_MONITORING)) {
						log.error("Job identified by name {} cannot be updated!",
								job.getName());
					...
					Job oldJ = DBUtility
							.getNamedQuerySingleResultByNamedParams(em,
									Job.QUERY_READ_JOB,
									Job.class, queryParams1);

					if (oldJ == null) {
						log.error("Job identified by name {} not found!",
								job.getName());
						throw new ConfigException("Job not found");
					}
