...
@NamedQueries({ @NamedQuery(
		name = JobInventoryNode.QUERY_COUNT_JOBNODE_WITH_NODE,
		query = "select count(jn) from JobInventoryNode jn where jn.node.name=:name") })
@XmlRootElement(namespace = "http://italtel.com/OaM/conf")
@XmlAccessorType(XmlAccessType.FIELD)
public class JobInventoryNode implements OMEntity {

	private static final long serialVersionUID = -741610482278385814L;

	public static final String QUERY_COUNT_JOBNODE_WITH_NODE = "countJobNodeWithNode";

	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE,
			generator = "SEQ_JOBNODE")
	@SequenceGenerator(name = "SEQ_JOBNODE",
			sequenceName = "SEQ_JOBNODE", initialValue = 0,
